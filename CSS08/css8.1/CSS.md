...
**패딩, 마진, 테두리는 각각 위, 아래, 좌, 우 속성을 따로따로 지정해줄 수 있다.**
...
개발자도구(F12)로 확인한 결과 Emotion은 마진값을 설정해준적이 없지만 값이 설정되어 있는 반면,
`span`태그는 마진값이 0으로 되어있다.

이 차이는 HTML 요소의 **기본 스타일** 때문에 발생하는 현상인데

`h1`태그는 **블록 요소**이다. 브라우저는 `h`에 기본적으로 위아래로 **마진값**을 설정한다.
`span` 태그는 **인라인 요소**이다. 본질적으로 주변 요소와 한 줄에 배치되며, 기본적으로 **마진값이 적용되지 않는다.** CSS로 명시적으로 마진을 지정해야 간격을 줄 수 있다.
...
오늘날 다양한 태블릿과 스마트폰이 있다. 이에 따라 화면 크기도 제각각이고 웹 페이지를 디자인할 때 어떻게 다양한 화면 크기에 대응할 수 있을까?? 라는 의문점이 있다면

해결책은 반응형 디자인이다. 화면 해상도에 따라 HTML과 CSS를 자동으로 맞춰서 보여주는 기능을 말한다.

반응형 디자인은 '미디어 쿼리'를 사용하면 되는데 사이트에 접속하는 미디어 타입과 특징에 따라 다른 CSS 스타일을 적용하도록 도와주는 규격이다.

이렇게 웹 사이트에 접속하는 기기 해상도에 따라서 HTML과 CSS가 바뀌는 웹을 반응형 웹이라고 부른다!

미디워 커리는 다음과 같은 상황에 사용할 수 있다.

- CSS에서 @media와 @import를 사용해 특정 조건에 따라 스타일을 적용할 때
- HTML 요소에 media 특성을 사용해 특정 매체만 가리킬 때

미디어 쿼리는 `@media`로 쓴다. 예를 들어 어떤 값에 따라 색깔을 다르게 적용할 때 다음과 같이 쓸 수 있다.

```html
@media (color) {...}
```

`@import`는 다른 스타일시트에서 스타일 규칙을 가져올 때 사용한다. 예를 들어 hello.css를 불러온다고 가정한다면 2가지 방법으로 작성할 수 있다.
...
이때 주의할 점은 `@import`는 문서 최상단에 있어야 한다. 이유는 코드는 위에서 아래로 실행되므로 사용할 파일을 최상단에서 먼저 불러와야 불러온 파일 내용을 이용하는 코드가 동작할 수 있다.
...
```css
body<!--1--> { 
  background-color<!--2-->: red<!--3-->
}
```

`1` 적용 대상 태그, `2` 적용할 속성, `3` 적용 값

<img src="css17.png" width=500>

실행 결과 배경색이 `pink`로 지정된 것을 확인할 수 있다.
...
미디어타입으로는 all(전부), print(프린트), screen(화면), speech(음성)를 지정할 수 있다.
아무것도 쓰지 않으면 기본값인 all(전부)이 적용된다.


미디어 쿼리를 적용하면,
```html
 <!--1--> 너비가 1280px 이하인 화면에서는 배경색이 핑크색
 @media (max-width: 1280px) {
  body {
    background-color: pink;
  }
 }
 <!--2--> 너비가 600px 이하인 화면에서는 배경색이 올리브색
...
```
`1`과 `2`로 미디어 쿼리를 이용해 색이 변하는 지점의 화면 너비 픽셀값을 지정한다.

**너비가 1280px 이하일 때 화면색** 
   
<img src="css19.png" width=700> 


**너비가 600px 이하일 때 화면색**

<img src="css18.png" width=500>


정리하면, `@media`를 작성한 후 `max-width` 속성을 사용했다. **max**는 최대라는 뜻이고, **width**는 너비 합치면
최대 너비이다. 그래서 1280px을 최대 너비로 지정했고 1280px까지는 핑크색 배경으로 보이는것이다.

1280px 이상이면 아무것도 지정하지 않았으니 하얀 배경이 나온다.

600px 이하에서는 올리브색 배경이 나온다.

정리하면, 미디어 쿼리를 사용하면 다양한 기기 해상도에 맞게 HTML과 CSS를 바꿀 수 있다.