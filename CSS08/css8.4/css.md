...
### 원리 : `first-child`는 반드시 부모가 있어야 한다.
...
위 구조에서는 p태그들이 부모 요소 없이 독립적으로 존재한다. 하지만 `p:first-child`는 선택자는 동작하지 않는다. 부모 요소가 없기 때문에 자식 요소로 간주 될 수 없고 부모가 없으므로, **"첫 아들 오준호"**를 지정할 수 없고, 비교할 **"형제 요소"**도 존재하지 않는다.
