...
텍스트나 이미지처럼 눈에 보이는 요소와 달리 마진과 패딩은 명확한 구분이 쉽지 않다. 마진이란 물건을 팔았을 때 남는 이익이다. 따라서 요소, 패딩, 테두리를 표시하고 남은 영역이 마진이라고 생각하면 된다. 우리는 추울 때 패딩을 입는다. 따라서 요소 위에 입는 것이 패딩이라고 생각하면 편하다.