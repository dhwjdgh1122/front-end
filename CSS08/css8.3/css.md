...
`position`값이 `relative`일 땐 자기 자신을 기준으로 움직였지만 `absolute`의 경우 가장 가까운 조상을 기준으로 움직인다. down 클래스 태그는 인접한 `up` 클래스는 부모가 아니기 때문에 기준이 될 수 없다.
 down 클래스 태그는 부모 요소 중 최상위 요소인 여백 영역을 기준으로 움직인다. 이 여백 영역을 기준으로 위쪽 20px만큼 움직인 것이다.

 정리하면,
  - 변경 전 `relative`일 때

    - `down` 클래스의 `position`이 `relative`라면, **자기 자신의 원래 위치**를 기준으로 이동한다.
    - 즉, `top: 20px;`을 적용하면, 원래 자리에서 20px만큼 아래로 이동한다.
---
- 변경 후 `absolute`일 때

  - `down` 클래스의 `position`을 `absolute`로 변경하면 **가장 가까운 `position`이 설정된 부모**를 기준으로 움직인다.
  - 그런데 `up`**클래스에는 `position` 속성이 없기 때문에, 기준이 될 수 없다.**
  - 결국 `body`(최상위 요소)를 기준으로 위치가 결정된다.
  - `top: 20px;`을 적용하면 `body`**를 기준으로 20px 이동하게 된다.**

  **즉, `absolute`로 변경하면 기존의 `up` 박스를 기준으로 움직이지 않고, 최상위 요소(body)를 기준으로 배치된다.
