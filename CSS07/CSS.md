...
`HTML` 문서를 시각적을 꾸미는 언어, 왜 HTML에 꾸미기 기능을 넣지 않고 `CSS`를 **별도**로 만들었을까?
...
HTML에서 p태그를 마감하기 전에 id="아이디" 형식으로 지정한다. 여기서 "아이디"는 중복 되지 않는 고유한 값이어야 한다. 주민번호라고 생각하면 편하다

아이디 선택자의 중요한 특징이 또 한 가지가 있다. CSS에서 아이디 선택자를 사용하여 스타일링을할 때는 #을 아이디 앞에 붙여야 한다는 점이다.
