...
아이디 선택자(ID selector)는 아이디로 요소를 식별해주는 선택자이다. 모든 태그 요소 중에 특정 아이디 하나를 선택할 떄 사용한다.
예를 들어 스타일을 적용할 p 태그에 아이디를 선언해 해당 선택자에 스타일링을 적용하는 것이다. p라는 태그 유형 대신 선언한 아이디로
요소를 선택하는 것이다.
